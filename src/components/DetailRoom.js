  ...
  render() {
    return (
       <div className="DatailRoom">
        ...
        <span className="song-detail"><h4 className="name-room-title">Mô tả: </h4> <p className="room-name-custom " style={{left: "157px",
    top: "-35px"}}>{this.props.room.nameSong}</p></span><br></br>
        {/* end */}
        <Link style={{marginRight: '25px'}} to={`InRoom/${this.props.room.name}/User`}>
        ...
        {this.props.room.status == "Wait" ? <Link to={`InRoom/${this.props.room.name}/MainUser2`}>
          <button onClick={this._Chien} className="btn btn-danger btn-detail">Tham gia</button>
        </Link> : ''}
